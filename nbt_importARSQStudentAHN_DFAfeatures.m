function nbt_importARSQStudentAHN_DFAfeatures(filename, SignalInfo, SaveDir)

%Include the mat file with DFA features on the same folder
...
%now adding DFA of loudness, pitch, rhythm and averaged
rsq.Questions{5,1}='DFA_Rhythm'
rsq.Questions{6,1}='DFA_Pitch'
rsq.Questions{7,1}='DFA_Loudness'
rsq.Questions{8,1}='DFA_Average'

%adding the answers stored in a matfile
acoustical_features=load('DFA_acousticfeaturesTemplate.mat');
DFAs=acoustical_features.DFArray; %16x5 cell

condition_order=SignalInfo.condition;
condition_template=DFAs(1:15,1);
ss=strcmp(condition_template,condition_order);
index=find(ss);


rsq.Answers(5,1)=DFAs{index,2}
rsq.Answers(6,1)=DFAs{index,3};
rsq.Answers(7,1)=DFAs{index,4};
rsq.Answers(8,1)=DFAs{index,5};
